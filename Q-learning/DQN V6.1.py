...
estimator = [estimator0, estimator1, estimator2]
...
class Environment(object):
    ...
    def step(self, state, weights, model, X, y):
        ...
        loss_clf = [log_loss(y_true, _) for _ in self.pred]     #真实概率和预测概率的损失
        ...
        y_pred = np.argmax(self.predictions, axis=1)         #取出数组中数值最大的索引
        # # 设计loss为预测正确和错误
...
class QLearningAgent:
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        with torch.no_grad():
            ...
            expected_q_value = reward + self.gamma * next_q_value * (1 - done) #贝尔曼公式
...
for i in range(len(X_test)):
    ...
    # Compute the accuracy of the predictions
    y_true = one_hot.transform(Y_test[i].reshape(-1, 1)).reshape(1, -1)
