...
# load data
train_data = pd.read_csv('..//data//精度1//E-train-1_删李31.csv')
...
def imbalanced_data_model():
    ...
    print('imbalanced_data_model:', accuracy_score(y_test, test_pred))
    return model
...
def ADASYN_model():
    ...
    xgb = XGBClassifier()
    xgb.fit(X_res, y_res)
    print('ADASYN_model:', accuracy_score(y_test, xgb.predict(X_test)))
    return xgb
...
for episode in tqdm(range(episodes)):
    score = train(agent, env, episodes, X_train)
    scores.append(score)
    ...
    acc.append(accuracy_score(y_th, y_predictions))
    print(accuracy_score(y_th, y_predictions))
