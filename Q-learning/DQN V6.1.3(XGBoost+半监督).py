...
# 划分数据集
x_train, x_test, y_train, y_test = train_test_split(data[features].values, LABEL, test_size=0.2, random_state=2023)
...
def imbalanced_data_model(X_train, X_test, y_train, y_test):
    # 合并所有数据
    # data = pd.concat((train_data, train_data1, test_data), axis=0)
    data = pd.concat((train_data, test_data), axis=0)
    features = ['SP', 'PE', 'GR', 'U', 'TH', 'K', 'AC', 'CNL', 'DEN', 'RLLS', 'RLLD']
    ...
    for label in y_train:
        if label == 0:
            ...
            weights.append(90)  # 类别1的样本权重为1
        elif label == 2:
            weights.append(180)  # 类别2的样本权重为2
        elif label == 3:
            weights.append(3)  # 类别3的样本权重为3
        elif label == 4:
            weights.append(150)  # 类别4的样本权重为15
        else:
    ...
    imbalanced_data = {'pre':test_pred, 'label':y_test}
    imbalanced_data = pd.DataFrame(imbalanced_data)
    imbalanced_data.to_csv('./plt/imbalanced_data.csv',index=False)

    print('imbalanced_data_model')
    ...
    ax.set_title('XGBoost+半监督')  # 标题
    ax.set_xlabel('Output Class')  # x轴
    ax.set_ylabel('Target Class')  # y轴
    plt.savefig('./plt/XGBoost+半监督.svg',bbox_inches='tight')
    plt.show()
    # print(C3)

    return model

def ADASYN_model(X_train, X_test, y_train, y_test):
    # 合并所有数据
    # data = pd.concat((train_data,train_data1, test_data), axis=0)
    data = pd.concat((train_data, test_data), axis=0)
    features = ['SP', 'PE', 'GR', 'U', 'TH', 'K', 'AC', 'CNL', 'DEN', 'RLLS', 'RLLD']
    ...
    ADASYN_data = {'pre': test_pred, 'label': y_test}
    ADASYN_data = pd.DataFrame(ADASYN_data)
    ADASYN_data.to_csv('./plt/ADASYN_data.csv', index=False)

    from sklearn.metrics import confusion_matrix
    import seaborn as sns
    import matplotlib.pyplot as plt
    C2 = confusion_matrix(y_test, xgb.predict(X_test), labels=[0, 1, 2, 3, 4, 5], sample_weight=None)
    print(C2)
    sns.set(font="simhei")  # 遇到标签需要汉字的可以在绘图前加上这句
    f, ax = plt.subplots()
    C3 = sns.heatmap(C2, annot=True, cmap="Blues", ax=ax, fmt='g',
    ...
    ax.set_xlabel('Output Class')  # x轴
    ax.set_ylabel('Target Class')  # y轴
    plt.savefig('./plt/XGBoost+ADASYN.svg',bbox_inches='tight')
    plt.show()
    # print(C3)
    return xgb
...
models.append(imbalanced_data_model(X_train, X_test, y_train, y_test))
models.append(ADASYN_model(X_train, X_test, y_train, y_test))
...
print(recall_score(Y_test, y_predictions, average=None) * 100)
print(np.mean(recall_score(Y_test, y_predictions, average=None) * 100))

from sklearn.metrics import confusion_matrix
import seaborn as sns
import matplotlib.pyplot as plt
C2 = confusion_matrix(Y_test, y_predictions, labels=[0, 1, 2, 3, 4, 5], sample_weight=None)
print(C2)
sns.set(font="simhei")  # 遇到标签需要汉字的可以在绘图前加上这句
f, ax = plt.subplots()
C3 = sns.heatmap(C2, annot=True, cmap="Blues", ax=ax, fmt='g',
...
ax.set_xlabel('Output Class')  # x轴
ax.set_ylabel('Target Class')  # y轴
plt.savefig('./plt/RL_data.svg',bbox_inches='tight')
plt.show()
print(C3)

RL_data = {'pre': y_predictions, 'label': Y_test}
RL_data = pd.DataFrame(RL_data)
RL_data.to_csv('./plt/RL_data.csv', index=False)
