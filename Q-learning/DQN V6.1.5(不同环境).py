...
# load data
train_data = pd.read_csv('.//data//E-train-1.csv')
train_data1 = pd.read_csv('.//data//无标签126层数据（8口井）.csv')
test_data = pd.read_csv('.//data//E-test.csv')
...
class Environment(object):
    ...
    def step(self, state, weights, model, X, y):
        ...
        # Compute the reward
        reward = -loss_clfs - loss_clf  # 强化学习与基线模型的交叉熵损失
        # reward = -loss_clfs  # 只有强化学习的交叉熵损失
...
print(round(np.mean(precision_score(Y_test, y_predictions, average=None)), 3))
print(round(np.mean(recall_score(Y_test, y_predictions, average=None)), 3))
print(round(np.mean(f1_score(Y_test, y_predictions, average=None)), 3))
# from sklearn.metrics import confusion_matrix
# import seaborn as sns
# import matplotlib.pyplot as plt
...
# RL_data.to_csv('./plt(DQN V6.1.4)/3.RL_data.csv', index=False)
